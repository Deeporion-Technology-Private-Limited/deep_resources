...
const paginationContainerStyles = cva('flex items-center justify-center space-x-2', {
  variants: {
    size: {
...
const paginationButtonStyles = cva('px-3 py-1  h-full w-full flex items-center justify-center', {
  variants: {
    shape: {
...
const Pagination = forwardRef<HTMLDivElement, PaginationProps>(
  (
    ...
    return (
      <Box className={cn(paginationContainerStyles({ size }), className)} ref={ref} {...props}>
        <button
          onClick={() => handleClick(currentPage - 1)}
          disabled={currentPage === 1}
          className={cn(paginationButtonStyles({ shape, variant, disabled: currentPage === 1 }))}
        ...
        <button
          ...
          className={cn(paginationButtonStyles({ shape, variant, disabled: currentPage === totalPages }))}
          style={{ width: customButtonSize, height: customButtonSize }}
        >
