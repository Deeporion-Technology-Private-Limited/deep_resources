...
import { ChartsType } from "../Layout";

const chartStyle = cva(["w-full"], {
  variants: {
    variant: {
...
export const Chart = React.forwardRef<HTMLDivElement, ChartProps>(
  ({
    ...
    const mapToFixedYAxis = (values: number[]) =>
      values.map((val) => (val / maxValue) * fixedMaxValue);
                  ...
                  (_label, index) =>
                    ...
                    index !== yAxisLabels.length - 1 && (
                      <line
                        ...
                        x1="0"
                        y1={
                          height - (index / (yAxisLabels.length - 1)) * height
                        }
                        x2={width}
                        y2={
                          height - (index / (yAxisLabels.length - 1)) * height
          ...
          {region?.map((item) => (
            <div className="flex justify-center items-center gap-2" key={item}>
              <div
                ...
                style={{
                  backgroundColor: `${
                    item === "Region1" ? lineColor1 : lineColor2
                  }`,
                }}
              ></div>
