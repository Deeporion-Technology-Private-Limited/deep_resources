import { Box, DefaultButton, FavouriteCard, Tabs, Text } from "@/components";
import { cn } from "@/utils";
import { ButtonIconPosition } from "@/components/types";
import React, { ComponentProps, forwardRef, useState } from "react";
import { ICards, ILabels } from "@/components/types";
      ...
      <Box className="flex gap-5 items-center w-full p-1 flex-wrap">
        {labels?.map((label) => (
          <DefaultButton
            key={label.id}
            className={cn(
              "rounded-3xl w-fit capitalize bg-[#EBE3E0] text-[#3F271E]",
              {
...
export const OffersAndGiftCards = forwardRef<
  HTMLDivElement,
  OffersAndGiftCardsProps
>(
  ({
    labelsData,
    cardsData,
      ...
      <Box className="flex " {...props}>
        <Tabs
          ...
          tabs={[
            ...
            {
              content: (
                <GiftCard cardsArray={cardsData} labelsArray={labelsData} />
              ),
              label: "Gift Cards",
            },
          ]}
