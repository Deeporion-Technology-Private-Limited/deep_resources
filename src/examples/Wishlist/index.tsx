...
import { ButtonSize, ICardDetails } from "@/components/types";
import { VariantProps, cva } from "class-variance-authority";
import { cn } from "@/utils";
import { FindIconUrl } from "@/utils/Constant";
import { DownArrow } from "@/components/MenuItem/Icon/icon";

const wishListStyles = cva(["w-full", "flex", "flex-col", "gap-5"]);
        ...
        <Box className="px-10">
          <Box className="flex justify-between ">
            <Box className="w-fit flex items-center justify-center sm:gap-2 gap-1 ">
              <Text
                as={"span"}
                className="text-zinc-800 sm:text-[26px] text-[14px] font-bold font-['Poppins']"
              >
                Wishlist{" "}
              </Text>
              <Text
                as={"p"}
                className="text-zinc-400 text-wrap sm:text-xl font-normal text-[10px] font-['Poppins']"
              >
                {" "}
                {`${`(` + length + `)`}`} items
              </Text>
            ...
            <Box className="flex justify-end sm:gap-2 gap-1">
              <Text
                as={"p"}
                className="text-zinc-400 w-fit font-['Poppins'] sm:text-[20px] text-[14px] flex items-center"
              >
                Sort by:{" "}
              </Text>
              <Box className="flex items-center justify-center gap-2 relative">
                <Text className="w-fit font-['Poppins'] sm:text-[20px] text-[14px]">
                  Recently Added
                </Text>
                <Box
              ...
              {showDropdown && (
                <Box className="flex z-10 flex-col w-[16rem] px-5 py-4 rounded-lg bg-slate-50 shadow absolute mt-[45px]">
                  ...
                  <Box className="flex w-full h-full pt-5 border-t-2 px-2 gap-2 ">
                    ...
                    <Button hover onClick={handleSorting}>
                      Apply
                    </Button>
                  </Box>
                </Box>
                ...
                <Box className="flex flex-col gap-10 w-full  h-full">
                  ...
                  <Box className="flex w-full border-t-2 border-dotted py-4 px-10 justify-center items-center bg-white gap-4 ">
                    ...
                    <Button
                      ...
                      style={{
                        background:
                          "linear-gradient(to right, #BFA59A, #3F271E)",
                      }}
                      onClick={addToCart}
                    >
