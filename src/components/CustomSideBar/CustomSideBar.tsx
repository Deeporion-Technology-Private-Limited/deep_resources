...
const profileStyles = cva("w-[44px] h-[44px] text-[1.5rem]");
    ...
    return (
      <Box ref={ref} className={cn(navBarStyles(), className)} {...props}>
        <Box className="flex flex-col gap-[10px]">
          <Box>
            <img className={cn(logoStyles(), LogoStyle)} src={Logo} alt="" />
          </Box>
          <Box>
            {sidebarTabs.map((items, index) => (
              <Box
                ...
                className={cn(
                  `text-white ${sidebarActiveTab === index && "bg-white/10"} flex gap-[10px] pt-[12px] hover:bg-white/10 pr-[12px] pb-[12px] pl-[24px]`,
                  sidebarTabsStyle
                )}
              >
        ...
        <Box>
            ...
            <Box>
              {profileData.map((items, index) => (
                <Box
                  ...
                  className={cn(
                    `text-white ${profileActiveTab === index && "bg-white/10"} flex gap-[10px] pt-[12px] hover:bg-white/10 pr-[12px] pb-[12px] pl-[24px]`,
                    sidebarProfileTabsStyle
                  )}
                >
                  <img src={items.icon} alt="" />
          ...
          <Box
            className="py-5  flex  items-center gap-7"
            onClick={() => setProfileOpen(!profileOpen)}
          >
            <BasicProfileAvatar
              className={cn(profileStyles(), profileAvtarStyle)}
              name={profileName}
            />
            {profileName}
            <img
              src={arrowIcon}
              alt=""
              className={`${profileOpen && "-rotate-90"} text-white`}
            />
          </Box>
        </Box>
      </Box>
