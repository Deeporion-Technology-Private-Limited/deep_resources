...
const boxStyle = cva("flex flex-col");
...
export const ProductDetailCard = forwardRef<HTMLDivElement, ProductCardProps>(
  (
    { productCode, productDetails, productSpecifications, className, ...props },
    ref
  ) => {
    const [showMore, setShowMore] = useState<boolean>(false);

    const specificationsArray = Object.entries(productSpecifications);
    const displayedSpecifications = showMore
      ? specificationsArray
      : specificationsArray.slice(0, 8);

    return (
      <Box ref={ref} className={cn(productCardStyles(), className)} {...props}>
        <Text>
          <span className="font-semibold">Product Code: </span>
          {productCode}
        </Text>
        {Object.entries(productDetails).map(([key, value]) => (
          <Box className={`${boxStyle()}`} style={{ marginBlock: "0.5rem" }}>
        ...
        <Box className="grid grid-cols-2 gap-4">
          {displayedSpecifications.map(([key, value]) => (
            <Box key={key} className="flex flex-col border-b border-[#E8EBED]">
              <Text
                className="text-sm font-normal leading-6 text-left bg-#72787F capitalize"
                style={{ color: "#72787F" }}
              >
                {key}
              </Text>
