            ...
            <Box ref={ref} {...props}>
                ...
                <Box className="flex flex-col py-6">
                    ...
                    <Box className="flex flex-wrap w-full gap-4 pt-5">
                        {data?.map((item: any) => (
                            <Box key={item.id} className="flex flex-col gap-[0.5rem] cursor-pointer hover:scale-110 transition" onClick={() => handleClick(item.label)}>
                                <ProductCard
                ...
                <Box className="flex justify-end gap-2">
                    ...
                    <Box className="flex items-center justify-center gap-2 relative">
                        ...
                        <Box className="w-full h-fit cursor-pointer" onClick={handleDropdown}>
                            <DownArrow />
                        </Box>
                ...
                <Box className="flex gap-3 pt-4">
                    {showChips.length > 0 ? showChips.map((label) =>
                        <Chips
                    ...
                    {showChips.length > 0 && <Button className="bg-transparent text-zinc-400 w-[5rem] h-8 p-0 font-normal" onClick={clearAllChips}>clear all</Button>}
                </Box>
            </Box>
        )
    }
