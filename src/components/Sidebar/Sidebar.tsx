...
import { MenuItemSize, NavItems } from "../types";
    ...
    const renderSubMenu = (Obj: NavItems) => {
      return (
        <Box className="w-full mt-[4px]">
          {Obj.submenu &&
            Obj.submenu?.map((item: { menus: any; }) => (
              <Box
    ...
    return (
      <Box ref={ref} className={cn(navBarStyles(), className)} {...props}>
        <Box className="flex justify-between px-2">
          <SingleIconButton iconUrl={FindIconUrl("Close.svg")} className="p-0" onClick={handleClose} />
          {LogoIcon !== "" ? (
        ...
        {!userLogedIn && (
          <Box className="p-[10px_12px] bg-[#EBE3E0] flex items-center rounded-[8px]">
            ...
            <DefaultButton className="bg-[#3F271E] w-[85px]" onClick={handleLogin}>Login</DefaultButton>
          </Box>
        )}
