import { Box, BreadCrumb, Checkbox, Chips, DefaultButton, ProductCard, Text } from "@/components";
            ...
            <Box className="flex flex-col w-[16rem] px-5 py-4 rounded-lg bg-white shadow absolute mt-[35px]">
              {optionsArray.map((label) => (
                <Checkbox
                  ...
                  value={label}
                  aria-label={label}
                  labelClassname="text-zinc-800 text-base font-normal font-['Poppins']" checked={false}                />
              ))}
              <Box className="flex w-full h-full pt-5 border-t-2 px-2 gap-2 ">
                <DefaultButton className=" h-fit bg-transparent text-[#3F271E]">
                  Clear all
                </DefaultButton>
                <DefaultButton hover>Apply</DefaultButton>
              </Box>
            </Box>
          )}
        </Box>
