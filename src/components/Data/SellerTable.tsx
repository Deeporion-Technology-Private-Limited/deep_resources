...
const SellerTable: React.FC<CompInterface> = ({ obj, setObj }) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
        ...
        if (checked) {
            setObj(prevObj =>
                prevObj.map((item, i) =>
                    i === index ? { ...item, Checked: "In Stock", verification: "Verified"} : item
                )
            );
        } else {
            setObj(prevObj =>
    ...
    return (
        ...
        <table className="min-w-full">
            <thead>
                <tr>
                    {["Product", "Price", "Onboarding Date", "Verification Status", "Available"].map((item) =>
                        <th key={item} className=" p-[] w-[209.6px] font-medium font-sans text-left">{item}</th>
                    )}
                </tr>
            </thead>
            <tbody>
                {obj.map((item, index) => (
                    <tr key={index} className="border-t">
                        <td className="flex flex-col md:flex-row items-start md:items-center py-2 md:py-4 text-sm gap-2">
                            <img src={item.img} alt={item.Product} className="w-12 h-12 md:w-16 md:h-16" />
                        ...
                        <td className="text-sm py-2 md:py-4">
                            {item.verification === "Pending" ? (
                                <Chips className="py-1 px-3 bg-[#FEE2E2] text-[#DC2626] cursor-pointer" variant={ChipsTypes.not_active} children={"Pending"} />
                        ...
                        <td>
                           ...
                           <Checkbox
                            ...
                            <div className="cursor-pointer hover:border" onClick={() => handleDelete(index)}>
                                <GreyDeleteIcon />
                            </div>
                           </div>
                        </td>
