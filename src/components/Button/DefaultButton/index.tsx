...
const getButtonClasses = ({
  ...
  return cn(buttonStyles({ variant, size }), hoverClasses, backgroundColor, className);
};
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const buttonClasses = getButtonClasses({ variant, size, hover, disabled, iconPosition, className });
    ...
    return (
      ...
      >
        {iconUrl && iconPosition === ButtonIconPosition.Left && (
          <img src={iconUrl} alt="Icon" className={cn(hasText ? "mr-2" : "")} />
        )}
        <span className={cn(hasText ? "" : "sr-only")}>{children}</span>
        {iconUrl && iconPosition === ButtonIconPosition.Right && (
          <img src={iconUrl} alt="Icon" className={cn(hasText ? "ml-2" : "")} />
        )}
      </button>
    );
  }
